"""
Standalone iMET capacitance/flux heatmap sweep script.
...
def _meets_derivative_repair_improvement_gate(before_jump, after_jump):
    if not np.isfinite(before_jump) or before_jump <= 0.0:
        return False
    if not np.isfinite(after_jump):
        return False

    improvement = before_jump - after_jump
    rel_improvement = improvement / max(before_jump, np.finfo(float).eps)
    return (
        improvement >= DERIVATIVE_REPAIR_MIN_TARGET_ABS_IMPROVEMENT
        or rel_improvement >= DERIVATIVE_REPAIR_MIN_TARGET_REL_IMPROVEMENT
    )
            ...
            for (
                ...
                if key in swapped_windows:
                    ...
                    while True:
                        ...
                        _ext_after_target_jump = (
                            np.inf
                            if _ext_level_recheck is None
                            else _ext_level_recheck["jump"]
                        )
                        _ext_improvement = jump["jump"] - _ext_after_target_jump
                        _ext_rel_improvement = _ext_improvement / max(
                            jump["jump"], np.finfo(float).eps
                        )
                        _ext_target_fixed = (
                        ...
                        if (
                            _meets_derivative_repair_improvement_gate(
                                jump["jump"], _ext_after_target_jump
                            )
                            and (
                ...
                improvement = jump["jump"] - after_target_jump
                rel_improvement = improvement / max(
                    jump["jump"], np.finfo(float).eps
                )
                print(
                    f"{prefix}     candidate level {candidate_partner}: "
...
def plot_z_flux_sweeps(phi_vals, column_results, frequency_ghz):
    ...
    for ax, result in zip(axes.flat, valid_columns):
        ...
        ax.axvline(LABEL_VALID_PHI_MAX, color="0.65", linewidth=0.8, linestyle="--")
        ax.set_title(
            rf"$C={capacitance_pf:.1f}$ pF, "
            rf"$L_{{tot}}={l_tot_nh:.6f}$ nH, "
            rf"$Z={z_ohm:.4f}\ \Omega$"
        )
    ...
    fig.suptitle(rf"Tracked flux sweeps by $Z$ at {frequency_ghz} GHz", fontsize=14)
    fig.tight_layout()
    return fig
...
def main():
    ...
    for frequency_ghz, sweep_rows in FREQUENCY_SWEEP_DATA.items():
        n_points = len(sweep_rows)
        alpha_grid = np.full((n_flux, n_points), np.nan)
        chi_grid = np.full((n_flux, n_points), np.nan)
        crossing_records = []
        column_results = []
        z_vals = []
        ...
        print(
            f"\nComputing alpha/chi heatmaps and tracked flux-sweep crossings on "
            f"{n_points} capacitance points x {n_flux} flux points for "
            f"{frequency_ghz} GHz (< {MAX_OVERLAY_GAP_GHZ * 1e3:.0f} MHz gap)..."
        )

        for i, (capacitance_pf, l_tot_nh) in enumerate(sweep_rows):
            C_r = capacitance_pf * 1e-12
            L_tot = l_tot_nh * 1e-9
            E_C_r = charging_energy_ghz(C_r)
            L_c = BETA_FIXED * L_tot
            z_ohm = float(np.sqrt(L_tot / C_r))
            z_vals.append(z_ohm)
            ...
            if column is None:
                ...
                eta = avg_per_point * (n_points - (i + 1))
                print(
                    f"  {i + 1}/{n_points} points "
                    f"(failed at C={capacitance_pf:.1f} pF, L_tot={l_tot_nh:.6f} nH) | "
                    f"elapsed={format_duration(elapsed)} | eta={format_duration(eta)}"
                )
                continue

            column["beta"] = BETA_FIXED
            column["frequency_ghz"] = float(frequency_ghz)
            column["capacitance_pf"] = capacitance_pf
            column["l_tot_nh"] = l_tot_nh
            column["z_ohm"] = z_ohm
            alpha_grid[:, i] = column["alpha"]
            chi_grid[:, i] = column["chi"]
            column_results.append(column)
            column_crossings = detect_tracked_crossings(
                z_ohm, phi_vals, column["tracked_evals"], column["tracked_labels"]
            )
            for rec in column_crossings:
                rec["frequency_ghz"] = float(frequency_ghz)
                rec["capacitance_pf"] = capacitance_pf
                rec["l_tot_nh"] = l_tot_nh
                rec["z_ohm"] = z_ohm
                rec["beta"] = BETA_FIXED
            ...
            print(
                f"  {i + 1}/{n_points} points | "
                f"C={capacitance_pf:.1f} pF | "
                f"L_tot={l_tot_nh:.6f} nH | "
                f"Z={z_ohm:.4f} Ohm | "
                f"crossings={len(column_crossings)} | "
                f"point_time={format_duration(point_elapsed)} | "
                f"elapsed={format_duration(elapsed)} | eta={format_duration(eta)}"
            )
        ...
        ax_alpha.set_xlabel(r"$Z=\sqrt{L_\mathrm{tot}/C_r}$ ($\Omega$)")
        ...
        ax_chi.set_xlabel(r"$Z=\sqrt{L_\mathrm{tot}/C_r}$ ($\Omega$)")
        ...
        ax_alpha_only.set_xlabel(r"$Z=\sqrt{L_\mathrm{tot}/C_r}$ ($\Omega$)")
        ...
        ax_chi_only.set_xlabel(r"$Z=\sqrt{L_\mathrm{tot}/C_r}$ ($\Omega$)")
        ...
        ax_abs_chi.set_xlabel(r"$Z=\sqrt{L_\mathrm{tot}/C_r}$ ($\Omega$)")
        ...
        ax_abs_chi_only.set_xlabel(r"$Z=\sqrt{L_\mathrm{tot}/C_r}$ ($\Omega$)")
        ...
        if crossing_records:
            z_cross = np.array([rec["z_ohm"] for rec in crossing_records], dtype=float)
        ...
        if crossing_records:
            print(
                f"\nTracked-state crossing overlay points for {frequency_ghz} GHz "
            ...
            for rec in crossing_records:
                print(
                    "  "
                    f"freq={rec['frequency_ghz']:.3f} GHz, "
                    f"C={rec['capacitance_pf']:.1f} pF, "
                    f"L_tot={rec['l_tot_nh']:.6f} nH, "
                    f"Z={rec['z_ohm']:.4f} Ohm, "
                    f"phi={rec['phi']:.6f}, "
                    f"{rec['kind']}, "
            ...
            print(f"\nTracked-state crossing overlay points for {frequency_ghz} GHz: none found.")
        ...
        fig_heatmaps.suptitle(
            rf"$\alpha$, $\chi$, and $|\chi|$ heatmaps at {frequency_ghz} GHz",
            fontsize=14,
        )
        fig_heatmaps.tight_layout()
        fig.suptitle(
            rf"$\alpha$, $\chi$, and $|\chi|$ vs $Z$ and flux at {frequency_ghz} GHz",
            fontsize=14,
        )
        fig.tight_layout()
        sweep_fig = plot_z_flux_sweeps(phi_vals, column_results, frequency_ghz)

        freq_tag = frequency_ghz.replace(".", "p")
        out_dir = os.path.join("plot_output", f"capacitance_sweep_beta05_{freq_tag}ghz")
        os.makedirs(out_dir, exist_ok=True)
        sweep_csv_path = os.path.join(out_dir, "imet_alpha_chi_vs_z_flux.csv")
        crossings_csv_path = os.path.join(out_dir, "imet_alpha_chi_vs_z_flux_crossings.csv")
        heatmaps_only_path = os.path.join(out_dir, "imet_alpha_chi_vs_z_flux_heatmaps_only.png")
        sweep_plot_path = os.path.join(out_dir, "imet_alpha_chi_vs_z_flux_tracked_sweeps.png")
        out_path = os.path.join(out_dir, "imet_alpha_chi_vs_z_flux.png")
        ...
        if sweep_fig is not None:
            sweep_fig.savefig(sweep_plot_path, dpi=150, bbox_inches="tight")
